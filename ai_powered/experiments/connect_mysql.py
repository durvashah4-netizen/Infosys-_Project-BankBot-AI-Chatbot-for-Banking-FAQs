...
MYSQL_CONFIG = {
    ...
    "user": "root",
    "password": "root",
     "database": "bankbot",   # 👈 jo schema use karna hai
...
if conn is None:
    st.error("Database connection failed. Check username/password.")
    st.stop()
...
schema = st.selectbox(
    "Select Schema",
    [s.decode() if isinstance(s, bytes) else s for s in schemas]
)
